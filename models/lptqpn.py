...
class ConvectionDiffusionLoss(nn.Module):
    ...
    def forward(self, x, matrix):
        ...
        c_d = self.a * torch.diff(matrix,
                                  dim=1) + self.b1 * gradients_x + self.b2 * gradients_y + self.c * gradients_xx + self.c * gradients_yy
...
class LPTQPN(nn.Module):
    def __init__(self,
            ...
            if i > 0:
                ...
                self.decoder_levels.append(nn.Sequential(*[
                    TransformerBlock(dim=int(dim * 2 ** i), num_heads=heads[i],
                                     ffn_expansion_factor=ffn_expansion_factor, bias=bias) for _ in
                    range(num_blocks[i])]))
            else:
                self.decoder_levels.append(nn.Sequential(*[
        ...
        self.central = self._create_transformer_blocks(int(dim * 2 ** 3), num_blocks[3], heads[3], ffn_expansion_factor,
                                                       bias)

        self.refinement = self._create_transformer_blocks(int(dim * 2 ** 1), num_refinement_blocks, heads[0],
        ...
        self.last = nn.Sequential(
            nn.Conv2d(in_channels=out_channels, out_channels=out_channels, kernel_size=3, padding=1), Swish())
        ...
        for _ in range(4):
            self.params.append(
                nn.Sequential(nn.Conv2d(in_channels=20, out_channels=1, kernel_size=3, stride=1, padding=1), nn.ReLU()))
            self.p.append(nn.Parameter(torch.ones(1)))

    def forward(self, inp_img):
        inp_img = inp_img.squeeze(2)
        ...
        # print("add_lastframe", add_lastframe.shape)
    ...
    def _create_transformer_blocks(self, dim, num_blocks, num_heads, ffn_expansion_factor, bias):
        return nn.Sequential(*[
            TransformerBlock(dim=dim, num_heads=num_heads, ffn_expansion_factor=ffn_expansion_factor, bias=bias) for i
            in range(num_blocks)])


if __name__ == '__main__':
    model = LPTQPN(inp_channels=5, out_channels=3)
    x = torch.ones(1, 5, 1, 256, 256)
    x = model(x)
    print(x.shape)
