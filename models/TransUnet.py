...
def get_r50_b16_config_n(output_frames):
    """Returns the Resnet50 + ViT-B/16 configuration."""
    config = get_b16_config()
    config.patches.grid = (16, 16)
    config.resnet = ml_collections.ConfigDict()
    ...
    config.decoder_channels = (256, 128, 64, 16)
    config.skip_channels = [512, 256, 64, 16]
    config.n_classes = output_frames
    config.n_skip = 3
    config.activation = 'softmax'
...
class VisionTransformer(nn.Module):
    def __init__(self, config, img_size=256, num_classes=4, zero_head=False, vis=False):
...
CONFIGS = {
    ...
    'R50-ViT-B_16_3': get_r50_b16_config_3(),
    'R50-ViT-B_16_20': get_r50_b16_config_20(),
...
TransUnet3 = VisionTransformer(config=CONFIGS['R50-ViT-B_16_3'])
TransUnet20 = VisionTransformer(config=CONFIGS['R50-ViT-B_16_20'])


def create_TransUnet(output_frames):
    return VisionTransformer(config=get_r50_b16_config_n(output_frames))
