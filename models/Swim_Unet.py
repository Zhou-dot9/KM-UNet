...
# def create_swin_unet(config_path='/home/ubuntu/zj/KM-UNetV3/other_models/swin_tiny_patch4_window7_224_lite.yaml'):
#     # 生成参数对象
#     parser = argparse.ArgumentParser()
#     # 添加所有必要的参数定义（与原始parser一致）
#     parser.add_argument('--cfg', type=str, default=config_path, metavar="FILE")
#     # 其他参数保持默认值
#     args = parser.parse_args([])  # 不接收命令行参数
...
if __name__ == '__main__':
    # 初始化模型
    model = SwinUnet(config, img_size=config.DATA.IMG_SIZE, num_classes=config.MODEL.NUM_CLASSES)

    # 示例输入
    x = torch.randn(4, config.MODEL.SWIN.IN_CHANS, 1,config.DATA.IMG_SIZE, config.DATA.IMG_SIZE)
    print(x.shape)
    output = model(x)
