...
class KANConv2d(nn.Module):
    ...
    def forward(self, x):
        ...
        # print('x.shape,before KANConv2d: ',x.shape)
        ...
        # print('x_transposed.shape: ', x_unfold.shape)
        x_unfold = x_unfold.reshape(batch_size * x_unfold.size(1), -1)
        # print('x_reshaped.shape, before KANLinear: ', x_unfold.shape)
        out_unfold = self.kanlayer(x_unfold)
        # print('x_out_unfold.shape, after KANLinear: ', out_unfold.shape)
        # Reshape and transpose to get the final output
        out_unfold = out_unfold.reshape(batch_size, -1, out_unfold.size(1))
        # print("x_reshaped2.shape: ",out_unfold.shape)
        out = out_unfold.transpose(1, 2)
        # print("x_transposed2.shape: ", out.shape)
