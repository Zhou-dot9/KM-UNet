...
class DirectionAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        # print(pooled.shape)
        weight = self.fc(pooled.view(B, C))

        # 局部增强
        q, k, v = self.qkv(x).chunk(3, dim=1)
        attn = torch.sigmoid(q * k) * v
...
class KM_UNetV3(nn.Module):
    def __init__(self, num_classes=3, embed_dims=[16, 32, 64]):
        ...
        # self.model_H1 = InterRowColSelfAttention(in_dim=embed_dims[0], q_k_dim=16, patch_ini=(128, 128), axis='H')
        # self.model_W1 = InterRowColSelfAttention(in_dim=embed_dims[0], q_k_dim=16, patch_ini=(128, 128), axis='W')
        # self.model_H2 = InterRowColSelfAttention(in_dim=embed_dims[1], q_k_dim=16, patch_ini=(64, 64), axis='H')
        # self.model_W2 = InterRowColSelfAttention(in_dim=embed_dims[1], q_k_dim=16, patch_ini=(64, 64), axis='W')
        # self.model_H3 = InterRowColSelfAttention(in_dim=embed_dims[2], q_k_dim=16, patch_ini=(32, 32), axis='H')
        # self.model_W3 = InterRowColSelfAttention(in_dim=embed_dims[2], q_k_dim=16, patch_ini=(32, 32), axis='W')
        # 编码器
        ...
        # self.wpl1=IntelligentWaveletPoolingModule()
    ...
    def forward(self, x):
        ...
        # print(e1.shape)
        e1=self.lca1(e1)
        # e1_h = self.model_H1(e1)
        # e1_w = self.model_W1(e1)
        # e1 = e1_h + e1_w

        e2 = self.enc2(e1)  # 输出通道数: embed_dims[1] (64)
        # print(e2.shape)
        e2=self.lca2(e2)
        # e2_h = self.model_H2(e2)
        # e2_w = self.model_W2(e2)
        # e2 = e2_h + e2_w

        e3 = self.enc3(e2)  # 输出通道数: embed_dims[2] (128)
        # print(e3.shape)
        e3=self.lca3(e3)
        # e3_h = self.model_H3(e3)
        # e3_w = self.model_W3(e3)
        # e3 = e3_h + e3_w

        # 解码路径
        d1 = self.dec1(e3)  # 输出通道数: embed_dims[1] (64)
